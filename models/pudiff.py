...
class PUNet(nn.Module):
    def __init__(self, npoint=1024, up_ratio=2, use_normal=False, use_bn=False, use_res=False):
        ...
        ## coordinate reconstruction by diffusion model
        in_ch = len(self.npoints) * 64 + 3  # 4 layers + input xyz
        self.diffusion=DiffusionPoint(
            net = PointwiseNet(point_dim=3, context_dim=in_ch, residual=True),
    ...
    def encode(self, points, npoint=None):
        if npoint is None:
        ...
        feats = torch.cat([
            ...
            l_feats[1],
            *up_feats], dim=1)  # bs, mid_ch, N, 1

        ## encode
        code = F.max_pool2d(
            feats, kernel_size=[1, feats.size(2)]).squeeze(-1)  # bs, mid_ch

        return code



    def get_pred_for_loss(self, input_data):
        code=self.encode(input_data)
        x_0 = input_data[..., :3].contiguous()
        return self.diffusion.get_pred_for_loss(x_0, code)


    def forward(self, points, npoint=None):
        code=self.encode(points,npoint)

        ## reconstruction
        output=self.diffusion.sample(self.npoint*self.up_ratio, code)
        return output
